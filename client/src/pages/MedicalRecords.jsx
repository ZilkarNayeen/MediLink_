...
import './Dashboard.css' // Uses shared dashboard styles
...
function MedicalRecords() {
  ...
  const [newRecord, setNewRecord] = useState({ title: '', recordType: 'lab_result', description: '', fileFile: null })
  ...
  const handleAddRecord = async () => {
    ...
    try {
      ...
      if (newRecord.fileFile) formData.append('document', newRecord.fileFile)
      ...
      setMessage({ text: 'Report added securely!', type: 'success' })
      setAddRecordModal(false)
      setNewRecord({ title: '', recordType: 'lab_result', description: '', fileFile: null })
      fetchRecords() 
    } catch (err) {
    ...
    <div className="page-wrapper" style={{ minHeight: '100vh', background: 'var(--bg)', paddingBottom: 'var(--bottom-nav-height)', paddingTop: 'var(--navbar-height)' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1.25rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem', marginBottom: '2rem' }}>
          <div>
            <h1 style={{ fontSize: 'clamp(1.4rem, 4vw, 1.8rem)', fontWeight: 800, color: 'var(--text)', marginBottom: '0.25rem' }}>Medical Reports</h1>
        ...
        {message.text && (
          <div className="ml-alert" style={{ marginBottom: '1rem', padding: '1rem', borderRadius: 'var(--radius)', backgroundColor: message.type === 'success' ? '#d1fae5' : '#fee2e2', color: message.type === 'success' ? '#065f46' : '#991b1b' }}>
            {message.text}
          </div>
        )}
        ...
        {!loading && filteredRecords.length > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.25rem' }}>
            {filteredRecords.map((r) => (
              <div key={r.id} className="ml-card" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column' }}>
                ...
                {r.fileUrl && (
                  <a href={`${API_BASE_URL.replace('/api', '')}${r.fileUrl}`} target="_blank" rel="noreferrer" className="ml-btn ml-btn-ghost" style={{ marginTop: 'auto', justifyContent: 'center', width: '100%' }}>
                    📎 View File
                  </a>
                )}
              </div>
      ...
      {addRecordModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', lg: 'blur(4px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }} onClick={() => setAddRecordModal(false)}>
          <div className="ml-card" style={{ width: '100%', maxWidth: 480, padding: '2rem' }} onClick={e => e.stopPropagation()}>
            <h2 style={{ fontSize: '1.3rem', fontWeight: 800, marginBottom: '0.5rem' }}>Upload Report</h2>
            ...
            <div className="ml-field mb-1">
              ...
              <input className="ml-input" type="file" onChange={e => setNewRecord({...newRecord, fileFile: e.target.files[0]})} />
            </div>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', marginTop: '2rem' }}>
              ...
              <button className="ml-btn ml-btn-primary" style={{ flex: 2 }} onClick={handleAddRecord} disabled={submitting || !newRecord.title}>
                {submitting ? 'Uploading…' : 'Save Report'}
              </button>
            </div>
          </div>
        </div>
